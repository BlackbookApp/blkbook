...
export function useUpdateExchangeNote() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ exchangeId, note }: { exchangeId: string; note: string }) =>
      updateExchangeNoteAction(exchangeId, note),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['exchanges'] });
    },
  });
